...
def run_within_task_cv(X, Y, n_splits=4, n_iter=10, use_pca=True):
    ...
    steps.append(("model", LogisticRegression(solver='saga', class_weight='balanced', multi_class=multiclass)))
    pipe = Pipeline(steps)
    ...
    return search.best_estimator_, search.best_score_, all_scores


if __name__ == "__main__":

    SAVE_DIR = "/mnt/scratch/projects/rewardMap/reward_signature/model_outputs_logreg_multinomial"
    ...
    for task in tasks:
        ...
        best_model, best_score, all_scores = run_within_task_cv(X_train, y_train, n_iter=3, n_splits=4, use_pca=use_pca)
